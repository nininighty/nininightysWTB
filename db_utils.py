    ...
    with db_cursor() as (conn, cursor):
        ...
        sql = f"""
            SELECT wt.id, wt.title, wt.init_score, wt.is_flippable, wt.file_path, wt.answer_file_path,
                   wt.last_review_date, wt.current_correct_rate,
            ...
            GROUP BY wt.id, wt.title, wt.init_score, wt.is_flippable, 
                     wt.file_path, wt.answer_file_path, wt.last_review_date, wt.current_correct_rate
        """
        ...
        for row in rows:
            ...
            total_weight = memory_weight * tag_weight * correct_weight * (1.5 - 0.75*row['init_score'])
            result.append({
                "id": row['id'],
                "weight": total_weight,
